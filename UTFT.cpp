...
void UTFT::fillScr(word color)
{
	...
	//Serial.println(">> ch: " + String(ch)+ ", cl: "+ String(cl));
...
byte UTFT::utf8ascii(byte ascii) {
...
// Code form http://playground.arduino.cc/Main/Utf8ascii
String UTFT::utf8ascii(String s) {
        String r= "";
        char c;
...
void UTFT::drawString(String text, int x, int y, int deg) {
  text = utf8ascii(text);
 
  int startX = 0;
  int startY = y;

  if (myTextAlignment == LEFT) {
    startX = x;
  } else if (myTextAlignment == CENTER) {
    int width = getStringWidth(text);
    startX = x - width / 2;
  } else if (myTextAlignment == RIGHT) {
    int width = getStringWidth(text);
    startX = x - width;
  }

  print(text, startX, startY, deg);
}

void UTFT::drawStringMaxWidth(String text, int x, int y, int maxLineWidth, int deg) {
  int currentLineWidth = 0;
  int startsAt = 0;
  int endsAt = 0;
  int lineNumber = 0;
  char currentChar = ' ';
  int lineHeight = getFontYsize();
  String currentLine = "";
  for (int i = 0; i < text.length(); i++) {
    ...
    if (currentChar == ' ' || currentChar == '-') {
      ...
      if (getStringWidth(lineCandidate) <= maxLineWidth) {
        ...
        drawString(text.substring(startsAt, endsAt), x, y + lineNumber * lineHeight, deg);
        lineNumber++;
        startsAt = endsAt + 1;
      }
    }
  ...
  drawString(text.substring(startsAt), x, y + lineNumber * lineHeight, deg);
}

int UTFT::getStringWidth(String text) {
  text = utf8ascii(text);
  //Serial.println(text + ", text.length:"+ String(text.length()) + ", FontXsize:"+ String(getFontXsize()));
  return text.length() * getFontXsize();
}

void UTFT::setTextAlignment(int textAlignment) {
...
void UTFT::drawXbm(int x, int y, int width, int height, const char *xbm) {
  if (width % 8 != 0) {
    width =  ((width / 8) + 1) * 8;
  }
  //cbi(P_CS, B_CS);
